...
**120 тестов**, сеть не используется (все внешние вызовы заглушены):

| Модуль | Что покрывает |
|---|---|
| `test_api.py` | эндпоинты, фильтры, заголовки безопасности, PWA-маршруты, 404/422 |
| `test_data.py` | схемы данных, координаты, согласованность каталога/квиза со словарями кода, файлы картинок типов |
...
| `test_weather.py` | WMO-коды, разбор ответа Open-Meteo, день недели, кэш, фильтр по городу, падение сети |
