...
```json
{"ok": true, "name": "Крым.Гид", "version": "1.0.0"}
```
...
Все поля обязательны, кроме `hours` (часы работы): оно приходит только
у объектов с публикуемым расписанием — дворцов, музеев, пещер и парков.
У пляжей и природных точек поля в ответе нет.
