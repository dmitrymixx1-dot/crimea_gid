...
```json
{"ok": true, "name": "Крым.Гид", "version": "1.6.0"}
```
...
| Ручка | Лимит | Окно | Env |
|---|---|---|---|
| `POST /api/quiz/evaluate` | 30 | 60 с | `RATE_LIMIT_QUIZ` |
| `GET /api/news?refresh=1` | 6 | 300 с | `RATE_LIMIT_REFRESH` |
| `GET /api/weather?refresh=1` | 6 | 300 с | `RATE_LIMIT_REFRESH` |
| `GET /api/sea?refresh=1` | 6 | 300 с | `RATE_LIMIT_REFRESH` |
...
X-RateLimit-Reset: 40        # секунд до полного очищения окна
...
| 429 | превышен лимит на дорогую ручку (`POST /api/quiz/evaluate`, `?refresh=1`); ответ несёт `Retry-After` |
