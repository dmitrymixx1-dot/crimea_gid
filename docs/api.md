...
**Ответ 422** — невалидные типы полей (например, `purpose` не список).
...
| Параметр | Тип | По умолчанию | Описание |
|---|---|---|---|
| `city` | string | — | id города (напр. `yalta`); неизвестный id → все города |
...
| Код | Когда |
|---|---|
| 404 | неизвестный `/api/*` маршрут; `/api/attractions/{id}` с несуществующим id |
| 405 | метод не поддерживается маршрутом (например, POST на GET-эндпоинт) |
| 422 | невалидные параметры запроса/тела (тип, диапазон `limit` и т.п.) |
