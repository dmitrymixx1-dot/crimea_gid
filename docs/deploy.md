...
запускают её по-настоящему. Репозиторий без подключённого сервера — это
состояние, а не ошибка: если ни один секрет `DEPLOY_*` не задан, пуш тега
проходит preflight и репетицию, а выкат пропускается с `notice` — релиз
остаётся зелёным. Частично заданные секреты — наоборот, ошибка: значит,
сервер уже задуман, но настроен криво. В ручном режиме `dry_run=true`
отсутствующие секреты становятся предупреждением, а не ошибкой.
