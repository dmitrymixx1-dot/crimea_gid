...
def test_reasons_are_translatable(bundle):
    ...
    # Готовые фразы берём из модуля, а не списком в тесте: новая причина,
    # забывшая про словарь, обязана валить тест (так было бы и с «Премиум-
    # впечатлением», если бы список жил здесь копией).
    for phrase in recommend.REASON_PHRASES:
