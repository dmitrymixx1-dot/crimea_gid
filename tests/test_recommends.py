...
def test_itinerary_reserve_goes_beyond_days():
    recs = [
        {"area": "A", "duration_h": 2, "tags": [], "id": f"a{i}", "name": f"A{i}"}
        for i in range(10)
    ]
...
def test_itinerary_stops_per_day_cap():
    ...
    recs = [
        {"area": "A", "duration_h": 3, "tags": [], "id": f"a{i}", "name": f"A{i}"}
        for i in range(4)
    ]
    r = plan_itinerary(recs, "1-2")
    for d in r["days"]:
